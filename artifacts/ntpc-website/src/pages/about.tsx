  ...
  return (
    <div className="w-full">
      ...
      <section className="relative h-[40vh] min-h-[280px] flex items-center justify-center overflow-hidden">
        <img src={congregationPhoto} alt="Congregation" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/60" />
        <div className="relative z-10 text-center text-white px-4">
          <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4">
            {language === 'am' ? 'ስለ እኛ' : 'About Us'}
          </h1>
          <p className="text-lg text-white/80">Ethiopian New Testament Priests Church</p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
              {[
                { title: language === 'am' ? 'ቃሉ' : 'The Word', desc: language === 'am' ? 'መጽሐፍ ቅዱስ የሕይወታችን ምሰሶ ነው' : 'Scripture is the foundation of everything we do' },
                { title: language === 'am' ? 'ጸሎት' : 'Prayer', desc: language === 'am' ? 'ጸሎት የቤተ ክርስቲያናችን ትንፋሽ ነው' : 'Prayer is the breath of our church — we are a house of prayer' },
                { title: language === 'am' ? 'ፍቅር' : 'Love', desc: language === 'am' ? 'አንዱ ለሌላው ከሚያሳዩት ፍቅር ይታወቃሉ' : 'We are known by the love we have for one another' },
              ].map((v, i) => (
                <div key={i} className="text-center p-6 rounded-xl border bg-card hover:shadow-md transition-shadow">
                  <h3 className="text-xl font-serif font-bold mb-3 text-primary">{v.title}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              <div className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-4">
                {language === 'am' ? 'ፓስተር / ሊቀ ጳጳስ' : 'Founding Pastor / Visionary'}
              </div>
              <h3 className="text-3xl font-serif font-bold mb-2">
                {language === 'am' ? 'ፓስተር ሚካኤል' : 'Pastor Michael'}
              </h3>
              <p className="text-muted-foreground mb-6 leading-relaxed">
                {language === 'am'
                  ? 'ፓስተር ሚካኤል ለሀያ ከሚልቀቁ ዓመታት ለክርስቶስ ሕይወቱን ወስኗል። ቤተ ክርስቲያናችን ሰፊ ጉዞ አድርጋ ዛሬ ወደዚህ ደረጃ የደረሰችው በፀሎቱ፣ ሕይወቱና አስተምህሮው ነው። "ለኔ ህይወት ክርስቶስ ነዉ" የሚለው ቃሉ የቤተ ክርስቲያናችን ዋና ጥቅስ ነው።'
                  : 'Pastor Michael has dedicated his life to Christ for over two decades. A man of deep prayer and uncompromising faith, he founded NTPC with a burning heart to see Ethiopia transformed by the power of the gospel. His signature message — "For me to live is Christ" (Philippians 1:21) — has become the DNA of this church family.'}
              </p>
              <p className="text-muted-foreground leading-relaxed">
                {language === 'am'
              ...
              <div className="mt-8 grid grid-cols-3 gap-6 text-center">
                {[
                  { num: '20+', label: language === 'am' ? 'ዓመት አገልግሎት' : 'Years in Ministry' },
                  { num: '10K+', label: language === 'am' ? 'አባላት' : 'Members' },
                  { num: '7', label: language === 'am' ? 'ክፍሎች' : 'Departments' },
                ].map((stat, i) => (
                  <div key={i} className="bg-primary/5 rounded-xl p-4 border border-primary/10">
                    <div className="text-2xl font-serif font-bold text-primary">{stat.num}</div>
      ...
      <section className="relative h-[50vh] overflow-hidden">
        <img src={buildingPhoto} alt="Church Building" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
          <div className="text-center text-white px-4">
            ...
            <p className="text-white/80 text-lg">Gerji, Addis Ababa, Ethiopia</p>
          </div>
        </div>
      </section>
    </div>
